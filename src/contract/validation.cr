module Contract
  module Validation
    TYPES = [
      Nil, Bool, Int32, Int64, Float64, String, Array(Value), Hash(String, Value),
      Range(Int32, Int32), Regex, Array(String), Array(Int32),
    ]
    ...
    private macro __process_params
      ...
      def initialize(@params : Hash(Contract::Key, Value), key : String)
        {% for name, options in FIELD_OPTIONS %}
          ...
          {% if field_type.is_a?(Generic) %}
            ...
            @{{name.id}} = @params[param_key].as({{field_type}}).map do |item|
              Contract::Cast.convert!(item, {{sub_type.join('|').id}}).as({{sub_type.join('|').id}})
            end
          {% else %}
            @{{name.id}} = Contract::Cast.convert!(@params[param_key], {{field_type}}).as({{field_type}})
          {% end %}
