...
class Array(T)
  ...
  def to_http_param(builder : HTTP::Params::Builder, key : String? = nil, converter : C = nil) : Nil forall C
    {% begin %}
      {% scalar = T.annotation(Schema::Definition::Scalar) || (T.union? && T.union_types.all? { |t| t.annotation(Schema::Definition::Scalar) || t == Nil }) %}
      ...
      query.split('&').each do |param|
        ...
        when "[]"
          {% if scalar %}
            ...
            begin
              ...
              {% if T.nilable? %}
                raise Schema::Definition::ParamTypeCastError.new(new_path, T, value.inspect) unless value.empty?
                instance << nil
              {% else %}
                raise Schema::Definition::ParamTypeCastError.new(new_path, T, value.inspect)
              {% end %}
            end
          {% else %}
            raise Schema::Definition::EmptyIndexForNonScalarArrayParamError.new(path, [""])
          {% end %}
        when /^\[(?<index>.*)\](?<nested>\[.+)/
          {% if scalar %}
            raise Schema::Definition::NestedContentForScalarParamError.new(path, [$~["index"]] + Schema::Definition.split_path($~["nested"]), {{@type}})
          {% else %}
            if $~["index"].empty?
              raise Schema::Definition::EmptyIndexForNonScalarArrayParamError.new(path, [$~["index"]] + Schema::Definition.split_path($~["nested"]))
            else
              begin
                index = $~["index"].to_i32
              rescue ArgumentError
                raise Schema::Definition::InvalidParamIndexError.new(path + {$~["index"]}, $~["index"].inspect)
              end
        ...
        when /^\[(?<index>.+)\]$/
          ...
          {% if scalar %}
            ...
            if index < 0
              raise Schema::Definition::ParamIndexOutOfBoundsError.new(new_path, index, instance.size)
          ...
          {% else %}
            raise Schema::Definition::ExplicitKeyForNonScalarParam.new(new_path, T)
          {% end %}
        end
      end
  ...
  private macro parse(type, converter, value, path)
    {% scalar = type.resolve.annotation(Schema::Definition::Scalar) || (type.resolve.union? && type.resolve.union_types.all? { |t| t.annotation(Schema::Definition::Scalar) || t == Nil }) %}
