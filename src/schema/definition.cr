...
module Schema
  module Definition
    ...
    def to_http_param(builder : HTTP::Params::Builder, key : String? = nil)
      ...
      if var = @{{ivar.name}}
        {%
          key = ((ann = ivar.annotation(Schema::Param)) && ann[:key]) || (ivar.name.stringify.underscore[0..0] + ivar.name.stringify.camelcase[1..-1])
        %}
        ...
        {% if converter = ivar.annotation(Schema::Param) && ivar.annotation(Schema::Param)[:converter] %}
    ...
    macro included
      ...
      def self.from_query(query : String)
        from_http_param(query: URI.decode(query), path: Tuple.new)
      end

      def self.from_urlencoded(query : String)
        from_http_param(query: URI.decode(query), path: Tuple.new)
      end
      ...
      {% begin %}
        ...
        {% for ivar in @type.instance_vars %}
          ...
          {%
            ...
            if (ann = ivar.annotation(Schema::Param)) && ann[:key]
              keys = [ann[:key]]
            else
              keys = default_keys
            end
          ...
          when {{keys.map { |k| "/^\\[?#{k.id}\\]?(?<nested>\\[.+)/" }.uniq.join(", ").id}}
            {% if scalar %}
              ...
              raise NestedContentForScalarParamError.new(_http_params_path + { {{ivar.name.stringify}} }, Schema::Definition.split_path($~["nested"]), {{ivar.type}})
            {% else %}
